...
def cmtitle(page, new_format, prefix=None):
    if prefix is None:
        prefix = ''
    if new_format:
...
def old_upload(self, fileobj, filename, description, license='', ignore=False, file_size=None):
    ...
    postdata = upload.UploadFile('wpUploadFile', filename, file_size, fileobj, predata)

    wait_token = self.wait_token()
    while True:
        try:
            self.connection.post(self.host, '%sindex.php?title=Special:Upload&maxlag=%s' % (self.path, self.max_lag), data=postdata).read()
